...
$$
\begin{align}
&\frac{ \partial u_i }{ \partial t } + u_j\frac{ \partial u_i }{ \partial x_{j} } = -\frac{1}{\rho}\frac{ \partial P }{ \partial x_{i} } + \nu \frac{ \partial^2 u_i }{ \partial x_{j}\partial x_{j}}  \\
\Leftrightarrow \quad & \frac{ \partial (\overline{U_i}+u'_i) }{ \partial t } +(\overline{U_j}+u'_j)\frac{ \partial (\overline{U_i}+u'_i) }{ \partial x_j } = -\frac{1}{\rho}\frac{\partial(\overline P+p')}{\partial x_i}+\nu\frac{\partial^2(\overline U_i+u'_i)}{\partial x_j\partial x_j}
\end{align}
$$
...
$$
\begin{align}
(4): \; \frac{ \partial^{2} \overline{\left( \overline{U_{i}}+u'_{i} \right) } }{ \partial x_{j}\partial x_{j} } &= \frac{ \partial^{2} \overline{\overline{U_{i}}} }{ \partial x_{j}\partial x_{j} } +\cancelto{0}{\frac{ \partial^{2} \overline{u'_{i}} }{ \partial x_{j}\partial x_{j} } } \\
&=\frac{ \partial^{2} \overline{U_{i}} }{ \partial x_{j}\partial x_{j} }
\end{align}
$$
정리된 항을 다시 대입하고 이항하면 다음과 같은 레이놀즈 평균화된 나비에-스토크스 방정식이 나온다:
$$
\begin{align}
& \frac{ \partial \overline{U_{i}} }{ \partial t } + \frac{ \partial \overline{U_{i}U_{j}} }{ \partial x_{j} } +\frac{ \partial \overline{u'_{i}u'_{j}} }{ \partial x_{j} } = -\frac{1}{\rho}\frac{ \partial \overline{P} }{ \partial x_{i} } + \frac{\mu}{\rho}\frac{ \partial^{2} \overline{U_{i}} }{ \partial x_{j}\partial x_{j}}  \\
\Leftrightarrow \quad &\frac{ \partial \overline{U_{i}} }{ \partial t } +\frac{ \partial \overline{U_{i}U_{j}} }{ \partial x_{j} } =-\frac{1}{\rho} \frac{ \partial \overline{P} }{ \partial x_{i} } +\frac{\mu}{\rho}\frac{ \partial^{2} \overline{U_{i}} }{ \partial x_{j}\partial x_{j}} -\frac{ \partial \overline{u'_{i}u'_{j}} }{ \partial x_{j} }  \\
\Leftrightarrow\quad  &\rho \left( \frac{ \partial \overline{U_{i}} }{ \partial t } +\frac{ \partial \overline{U_{i}U_{j}} }{ \partial x_{j} } \right) =-\frac{ \partial \overline{P} }{ \partial x_{i} } +\mu \frac{ \partial^{2} \overline{U_{i}} }{ \partial x_{j}\partial x_{j}} -\frac{ \partial \rho\overline{u'_{i}u'_{j}} }{ \partial x_{j} }
\end{align}
$$
...
$$
\mu \frac{ \partial^{2} \overline{U_{i}} }{ \partial x_{j}\partial x_{j} }
$$
- 설명: 시간 평균 유속 $\overline{U_{i}}$ 의 유체 점성에 의한 운동량 확산을 나타낸 항이다.
...
$$
- 설명: 난류의 변동 성분 $u'_{i}$ 와 $u'_{j}$ 의 상관 관계에서 발생하는 추가적인 운동량 수송을 나타낸다. 여기서는 $R_{ij}=\overline{u'_{i}u'_{j}}$를 상관 텐서로 두며, 응력 단위의 레이놀즈 응력은 $\tau^R_{ij}=-\rho R_{ij}$이다.
- 의미: 이 항은 RANS 방정식에서 가장 중요한 항이자 난류 모델링의 "종결 문제(closure problem)"를 야기하는 핵심 항이다. 난류 유동에서는 속도의 불규칙한 변동으로 인해 추가적인 "겉보기 응력(apparent stress)"이 발생하며, 이것이 평균 유동에 미치는 영향을 나타낸다. 이 항은 순간적인 나비에-스토크스 방정식에는 없던 새로운 항이며, 평균화 과정을 통해 나타난다.
