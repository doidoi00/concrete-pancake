...
<img src="../Attached Files/dxdydz.svg" width = "50%"/>
...
<img src="../Attached Files/waterflow.svg" width = "50%"/>
...
\therefore \; \text{Net mass outflow rate} :\; \dot m_{\mathrm{out}}-\dot m_{\mathrm{in}}=\frac{\partial(\rho v_i)}{\partial x_i}dV
\end{align}
$$
...
$$
\begin{align}
\text{Net mass outflow rate} &= \dot m_{\mathrm{out}}-\dot m_{\mathrm{in}} \\
&= \frac{\partial(\rho v_i)}{\partial x_i}dV \\
&= \nabla \cdot (\rho\vec{v})\,dV
\end{align}
$$
Net mass flow는 **질량 플럭스** $\rho\vec{v}$의 발산을 나타낸다. 즉 $\nabla\cdot(\rho\vec{v})>0$이면 순 질량 유출, $\nabla\cdot(\rho\vec{v})<0$이면 순 질량 유입을 뜻한다. 밀도가 일정한 경우에만 이것을 $\nabla\cdot\vec{v}$의 부호로 바로 판단할 수 있다.
...
$$
\begin{align*}
\nabla \cdot (\rho\vec{v})\,dV + \frac{\partial \rho}{\partial t}dV &= 0 \\
\Rightarrow \nabla \cdot (\rho\vec{v}) + \frac{\partial \rho}{\partial t} &=0
\end{align*}
$$

비압축성 유체의 경우 물질입자를 따라 밀도가 변하지 않아 $D\rho/Dt=0$이다. 위 식을 물질미분 형태 $D\rho/Dt+\rho\nabla\cdot\vec{v}=0$로 쓰면, $\rho\ne0$에서:
$$
\nabla \cdot \vec{v} = 0
$$
...
$$
\nabla \cdot \vec{u} =\frac{\partial u_i}{\partial x_{i}}= \frac{\partial \bar{U}_i}{\partial x_{i}}+\frac{\partial u'_i}{\partial x_{i}} = 0
$$
- $\vec{u}$ : 유체의 유속
- $\bar{U}$ : 유속의 시간 평균값
- $u'$ : 유속의 변동 성분값
...
$$
$$
\therefore \frac{ \partial \overline{U_{i}} }{ \partial x_{i} }=0
$$

---
## 레이놀즈 평균화된 연속방정식의 의미

레이놀즈 평균화된 연속방정식은 평균 속도장의 발산이 0이라는 뜻이다. 순간 유동도 비압축성이라는 원래 가정과 평균 식을 함께 사용하면 $\partial u_i'/\partial x_i=0$도 따른다. 즉 변동 성분은 질량을 생성하거나 소멸시키지 않지만, 평균 운동량 수송에는 레이놀즈 응력으로 기여한다.
