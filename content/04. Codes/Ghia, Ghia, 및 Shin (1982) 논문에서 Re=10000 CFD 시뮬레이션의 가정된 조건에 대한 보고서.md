...
계산은 **2차원 비압축성 Navier--Stokes 방정식**을 난류 모형 없이 정상 해로 푸는 설정입니다. 이를 실제 고-Re 유동이 물리적으로 항상 층류라는 뜻으로 해석하면 안 됩니다. 이 모델은 2D 정상 해를 비교하는 수치 벤치마크입니다.1
...
- **미끄럼 없음 조건(no-slip):** 모든 고체, 비다공성 벽면에서는 미끄럼 없음 조건이 적용됩니다. 이는 이동하는 상단 벽을 제외하고는 모든 벽면에서 유체 속도 성분(u 및 v)이 0임을 의미합니다.1
